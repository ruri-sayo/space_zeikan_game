...
## 1. 前提
本実装は Astro + React コンポーネントとして構築済みで、既存Astroサイトへページ単位または埋め込み単位で移植可能。

## 2. UI構成（最新版）
- Home画面で Easy / Hard を選択
- 本編は 3カラム
  - 左: 申告書類・スペック（旧Documents）
  - 中央: 物理可視化（Orbit / Signal）
  - 右: 証言ログと監査チェック
- ヘッダーに「ホームに戻る」ボタン（確認ダイアログ付き）

## 3. 難易度差分
- Easy: ガイダンス表示あり、チェック項目にヒント文を表示
- Hard: ガイダンス非表示、チェック項目は抽象表示
...
### パターンC: iframe埋め込み
本機能のみ別デプロイし、既存ブログへ iframe 埋め込み。
- 推奨最小サイズ: 960x540
- モバイル時は2カラム崩し
...
5. 既存ルーティングへ `src/pages/index.astro` 相当を統合

## 6. データ運用
- 現状は `src/data/cases/*.json` の静的同梱。
- 将来CMS化する場合は、`caseLoader.ts` を API fetch に差し替えればUI/エンジンは流用可能。
